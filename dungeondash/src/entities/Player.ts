import Phaser from "phaser";
import Graphics from "../assets/Graphics";
import Weapon, { WEAPONS } from "./Weapon";
import { network } from "../network/NetworkManager";

const speed = 125;
const attackSpeed = 500;
const attackDuration = 165;
const staggerDuration = 200;
const staggerSpeed = 100;
const attackCooldown = attackDuration * 2;

interface Keys {
  up: Phaser.Input.Keyboard.Key;
  down: Phaser.Input.Keyboard.Key;
  left: Phaser.Input.Keyboard.Key;
  right: Phaser.Input.Keyboard.Key;
  space: Phaser.Input.Keyboard.Key;
  w: Phaser.Input.Keyboard.Key;
  a: Phaser.Input.Keyboard.Key;
  s: Phaser.Input.Keyboard.Key;
  d: Phaser.Input.Keyboard.Key;
  q: Phaser.Input.Keyboard.Key;
}

export default class Player {
  public sprite: Phaser.Physics.Arcade.Sprite;
  private keys: Keys;

  // Stats
  public hp: number;
  public maxHp: number;
  public armor: number;
  public maxArmor: number;
  public mana: number;
  public maxMana: number;
  public coins: number;

  // Weapons
  public weapons: Weapon[];
  public currentWeaponIndex: number;
  public bulletGroup: Phaser.Physics.Arcade.Group;

  // Combat
  private attackUntil: number;
  private staggerUntil: number;
  private attackLockedUntil: number;
  private invincibleUntil: number;
  private lastArmorDamageTime: number;
  private emitter: Phaser.GameObjects.Particles.ParticleEmitter;
  private flashEmitter: Phaser.GameObjects.Particles.ParticleEmitter;
  private body: Phaser.Physics.Arcade.Body;
  private attacking: boolean;
  private time: number;
  private staggered: boolean;
  private scene: Phaser.Scene;
  private facingUp: boolean;
  public dead: boolean;

  // Aim
  private aimLine: Phaser.GameObjects.Line | null;

  constructor(x: number, y: number, scene: Phaser.Scene) {
    this.scene = scene;
    this.sprite = scene.physics.add.sprite(x, y, Graphics.player.name, 0);
    this.sprite.setSize(8, 8);
    this.sprite.setOffset(20, 28);
    this.sprite.anims.play(Graphics.player.animations.idle.key);
    this.facingUp = false;
    this.sprite.setDepth(5);
    this.dead = false;

    // Stats (Knight defaults from GDD)
    this.hp = 7;
    this.maxHp = 7;
    this.armor = 7;
    this.maxArmor = 7;
    this.mana = 200;
    this.maxMana = 200;
    this.coins = 0;

    // Weapons - start with pistol
    this.weapons = [
      new Weapon(WEAPONS.pistol),
      new Weapon(WEAPONS.shotgun)
    ];
    this.currentWeaponIndex = 0;

    // Bullet pool
    this.bulletGroup = scene.physics.add.group({
      defaultKey: "bullet",
      maxSize: 50,
      createCallback: (obj: Phaser.GameObjects.GameObject) => {
        const s = obj as Phaser.Physics.Arcade.Sprite;
        s.setSize(4, 4);
      }
    });

    this.keys = scene.input.keyboard.addKeys({
      up: Phaser.Input.Keyboard.KeyCodes.UP,
      down: Phaser.Input.Keyboard.KeyCodes.DOWN,
      left: Phaser.Input.Keyboard.KeyCodes.LEFT,
      right: Phaser.Input.Keyboard.KeyCodes.RIGHT,
      space: Phaser.Input.Keyboard.KeyCodes.SPACE,
      w: "w",
      a: "a",
      s: "s",
      d: "d",
      q: "q"
    }) as Keys;

    this.attackUntil = 0;
    this.attackLockedUntil = 0;
    this.attacking = false;
    this.staggerUntil = 0;
    this.staggered = false;
    this.invincibleUntil = 0;
    this.lastArmorDamageTime = 0;

    const particles = scene.add.particles(Graphics.player.name);
    particles.setDepth(6);
    this.emitter = particles.createEmitter({
      alpha: { start: 0.7, end: 0, ease: "Cubic.easeOut" },
      follow: this.sprite,
      quantity: 1,
      lifespan: 200,
      blendMode: Phaser.BlendModes.ADD,
      scaleX: () => (this.sprite.flipX ? -1 : 1),
      emitCallback: (particle: Phaser.GameObjects.Particles.Particle) => {
        particle.frame = this.sprite.frame;
      }
    });
    this.emitter.stop();

    this.flashEmitter = particles.createEmitter({
      alpha: { start: 0.5, end: 0, ease: "Cubic.easeOut" },
      follow: this.sprite,
      quantity: 1,
      lifespan: 100,
      scaleX: () => (this.sprite.flipX ? -1 : 1),
      emitCallback: (particle: Phaser.GameObjects.Particles.Particle) => {
        particle.frame = this.sprite.frame;
      }
    });
    this.flashEmitter.stop();

    this.body = <Phaser.Physics.Arcade.Body>this.sprite.body;
    this.time = 0;
    this.aimLine = null;

    // Q key to switch weapons
    scene.input.keyboard.on("keydown-Q", () => {
      this.currentWeaponIndex =
        (this.currentWeaponIndex + 1) % this.weapons.length;
      this.scene.events.emit("weaponChanged", this.getCurrentWeapon());
    });

    // Mouse: fire on click
    scene.input.on("pointerdown", (pointer: Phaser.Input.Pointer) => {
      if (this.dead) return;
      this.tryShoot(pointer);
    });
  }

  getCurrentWeapon(): Weapon {
    return this.weapons[this.currentWeaponIndex];
  }

  tryShoot(pointer: Phaser.Input.Pointer) {
    const weapon = this.getCurrentWeapon();
    const worldPoint = this.scene.cameras.main.getWorldPoint(
      pointer.x,
      pointer.y
    );

    if (!weapon.canFire(this.time, this.mana)) return;

    const { manaCost, firedBullets } = weapon.fire(
      this.scene,
      this.bulletGroup,
      this.sprite.x,
      this.sprite.y,
      worldPoint.x,
      worldPoint.y,
      this.time
    );
    this.mana -= manaCost;
    this.scene.events.emit("playerStatsChanged");

    // Network: broadcast each bullet
    if (firedBullets.length > 0 && network.connected) {
      for (const b of firedBullets) {
        network.sendBulletFired(b.x, b.y, b.vx, b.vy, b.damage, "bullet");
      }
    }
  }

  takeDamage(amount: number) {
    if (this.dead) return;
    if (this.time < this.invincibleUntil) return;

    // Armor absorbs first
    if (this.armor > 0) {
      const armorDmg = Math.min(this.armor, amount);
      this.armor -= armorDmg;
      amount -= armorDmg;
      this.lastArmorDamageTime = this.time;
    }

    // Remaining goes to HP
    if (amount > 0) {
      this.hp -= amount;
    }

    // i-frames
    this.invincibleUntil = this.time + 500;

    // Visual feedback
    this.scene.cameras.main.shake(80, 0.001);
    this.scene.cameras.main.flash(50, 100, 0, 0);

    // Blink
    this.scene.tweens.add({
      targets: this.sprite,
      alpha: 0.3,
      duration: 80,
      yoyo: true,
      repeat: 3,
      onComplete: () => {
        this.sprite.setAlpha(1);
      }
    });

    this.scene.events.emit("playerStatsChanged");

    if (this.hp <= 0) {
      this.hp = 0;
      this.die();
    } else {
      this.hp = Math.ceil(this.hp);
    }
  }

  heal(amount: number) {
    this.hp = Math.min(Math.ceil(this.hp + amount), this.maxHp);
    this.scene.events.emit("playerStatsChanged");
  }

  addMana(amount: number) {
    this.mana = Math.min(this.mana + amount, this.maxMana);
    this.scene.events.emit("playerStatsChanged");
  }

  addCoins(amount: number) {
    this.coins += amount;
    this.scene.events.emit("playerStatsChanged");
  }

  die() {
    this.dead = true;
    this.body.setVelocity(0);
    this.sprite.anims.play(Graphics.player.animations.stagger.key);
    this.scene.events.emit("playerDied");
  }

  isAttacking(): boolean {
    return this.attacking;
  }

  stagger(): void {
    if (this.time > this.staggerUntil) {
      this.staggered = true;
      this.takeDamage(1);
    }
  }

  update(time: number, delta: number) {
    this.time = time;
    if (this.dead) return;

    const keys = this.keys;
    let attackAnim = "";
    let moveAnim = "";

    // Armor regen (3s after last damage)
    if (
      this.armor < this.maxArmor &&
      time > this.lastArmorDamageTime + 3000
    ) {
      this.armor = Math.min(
        this.armor + 0.5 * (delta / 1000),
        this.maxArmor
      );
      this.scene.events.emit("playerStatsChanged");
    }

    // Mana passive regen (slower)
    if (this.mana < this.maxMana) {
      this.mana = Math.min(this.mana + 5 * (delta / 1000), this.maxMana);
    }

    // Continuous fire while holding mouse
    if (this.scene.input.activePointer.isDown) {
      this.tryShoot(this.scene.input.activePointer);
    }

    // Kill bullets that are out of range
    this.bulletGroup.getChildren().forEach((b: Phaser.GameObjects.GameObject) => {
      const bullet = b as Phaser.Physics.Arcade.Sprite;
      if (!bullet.active) return;
      const dist = Phaser.Math.Distance.Between(
        (bullet as any).startX,
        (bullet as any).startY,
        bullet.x,
        bullet.y
      );
      if (dist > (bullet as any).maxRange) {
        bullet.setActive(false).setVisible(false);
        bullet.body.stop();
      }
    });

    if (this.staggered && !this.body.touching.none) {
      this.staggerUntil = this.time + staggerDuration;
      this.staggered = false;

      this.body.setVelocity(0);
      if (this.body.touching.down) {
        this.body.setVelocityY(-staggerSpeed);
      } else if (this.body.touching.up) {
        this.body.setVelocityY(staggerSpeed);
      } else if (this.body.touching.left) {
        this.body.setVelocityX(staggerSpeed);
        this.sprite.setFlipX(true);
      } else if (this.body.touching.right) {
        this.body.setVelocityX(-staggerSpeed);
        this.sprite.setFlipX(false);
      }
      this.sprite.anims.play(Graphics.player.animations.stagger.key);

      this.flashEmitter.start();
    }

    if (time < this.attackUntil || time < this.staggerUntil) {
      return;
    }

    this.body.setVelocity(0);

    const left = keys.left.isDown || keys.a.isDown;
    const right = keys.right.isDown || keys.d.isDown;
    const up = keys.up.isDown || keys.w.isDown;
    const down = keys.down.isDown || keys.s.isDown;

    if (!this.body.blocked.left && left) {
      this.body.setVelocityX(-speed);
      this.sprite.setFlipX(true);
    } else if (!this.body.blocked.right && right) {
      this.body.setVelocityX(speed);
      this.sprite.setFlipX(false);
    }

    if (!this.body.blocked.up && up) {
      this.body.setVelocityY(-speed);
    } else if (!this.body.blocked.down && down) {
      this.body.setVelocityY(speed);
    }

    if (left || right) {
      moveAnim = Graphics.player.animations.walk.key;
      attackAnim = Graphics.player.animations.slash.key;
      this.facingUp = false;
    } else if (down) {
      moveAnim = Graphics.player.animations.walk.key;
      attackAnim = Graphics.player.animations.slashDown.key;
      this.facingUp = false;
    } else if (up) {
      moveAnim = Graphics.player.animations.walkBack.key;
      attackAnim = Graphics.player.animations.slashUp.key;
      this.facingUp = true;
    } else if (this.facingUp) {
      moveAnim = Graphics.player.animations.idleBack.key;
    } else {
      moveAnim = Graphics.player.animations.idle.key;
    }

    if (
      keys.space!.isDown &&
      time > this.attackLockedUntil &&
      this.body.velocity.length() > 0
    ) {
      this.attackUntil = time + attackDuration;
      this.attackLockedUntil = time + attackDuration + attackCooldown;
      this.body.velocity.normalize().scale(attackSpeed);
      this.sprite.anims.play(attackAnim, true);
      this.emitter.start();
      this.sprite.setBlendMode(Phaser.BlendModes.ADD);
      this.attacking = true;
      return;
    }

    this.attacking = false;
    this.sprite.anims.play(moveAnim, true);
    this.body.velocity.normalize().scale(speed);
    this.sprite.setBlendMode(Phaser.BlendModes.NORMAL);
    if (this.emitter.on) {
      this.emitter.stop();
    }
    if (this.flashEmitter.on) {
      this.flashEmitter.stop();
    }
  }
}
